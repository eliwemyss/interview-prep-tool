...
async function getAllCompanies() {
  const query = `
    ...
    ORDER BY 
      CASE 
        WHEN next_interview IS NOT NULL THEN 0 
        ELSE 1 
      END,
      next_interview ASC NULLS LAST,
      updated_at DESC
  `;
...
async function addCompanyToPipeline(companyName, stage, nextInterview, notes) {
  const query = `
    INSERT INTO companies (name, stage, next_interview, notes, updated_at)
    VALUES ($1, $2, $3, $4, NOW())
    ON CONFLICT (name) 
    DO UPDATE SET 
      stage = $2,
...
async function addCalendarEvent(calendarEventId, companyId, eventTitle, eventDate) {
  const query = `
    INSERT INTO calendar_events (calendar_event_id, company_id, event_title, event_date)
    VALUES ($1, $2, $3, $4)
...
async function checkPrepEmailStatus(calendarEventId) {
  const query = 'SELECT prep_sent FROM calendar_events WHERE calendar_event_id = $1';
...
async function getUpcomingCalendarEvents(days = 7) {
  const query = `
    ...
    WHERE ce.event_date >= NOW() 
      AND ce.event_date <= NOW() + INTERVAL '${days} days'
