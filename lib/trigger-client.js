  ...
  async queueResearchJob(payload) {
    try {
      ...
      if (!this.isConfigured()) {
        console.log('[Trigger.dev] Not configured - returning mock response');
        return {
          success: false,
          queued: false,
          error: 'Trigger.dev not configured',
          message: 'Set TRIGGER_API_KEY and TRIGGER_PROJECT_ID environment variables'
        };
      }

      // Call Trigger.dev v3 API to trigger the company-research-job task
      const triggerUrl = `${this.apiUrl}/api/v1/tasks/${this.projectId}/company-research-job/trigger`;
      ...
      console.log(`✅ Trigger.dev job queued: ${response.data.id}`);
      ...
      return {
        ...
        message: 'Research job queued via Trigger.dev v3 for async processing'
      };
    } catch (error) {
