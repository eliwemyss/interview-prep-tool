...
/**
 * Trigger.dev API client for interview-prep-tool
 * Handles asynchronous job queuing and status tracking
 * Professional implementation showcasing job queue architecture
 */

const TRIGGER_API_URL = process.env.TRIGGER_API_URL || 'https://api.trigger.dev/v1';
...
class TriggerJobClient {
  ...
  /**
   * Get authorization headers for Trigger.dev API
   */
  getHeaders() {
    return {
   ...
   * @param {string} companyUrl - Company website
   * @param {string} role - Position being interviewed for
   * @returns {Promise<Object>} Job result or queuing confirmation
   */
  async queueResearchJob(companyName, companyUrl, role) {
    try {
      const jobPayload = {
        companyName,
        ...
        source: 'interview-prep-tool'
      };

      const jobId = `job_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

      console.log(`[Trigger.dev] Queueing research job: ${jobId} for ${companyName}`);
      ...
      return {
        ...
        message: this.isConfigured() 
          ? 'Research job queued for asynchronous processing via Trigger.dev'
          : 'Research job prepared (Trigger.dev not configured - would process asynchronously)'
      };
    } catch (error) {
      console.error('[Trigger.dev] Error queuing job:', error.message);
      return {
        success: false,
        queued: false,
        error: error.message
